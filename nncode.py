...
sys.path.append(os.path.join(os.path.dirname(__file__), "./manim/old_projects/nn"))
sys.path.append(os.path.join(os.path.dirname(__file__), "./manim/old_projects"))
...
DEFAULT_GAUSS_BLUR_CONFIG = {"ksize": (5, 5), "sigmaX": 10, "sigmaY": 10}

DEFAULT_CANNY_CONFIG = {"threshold1": 100, "threshold2": 200}
...
class WrappedImage(Group):
    CONFIG = {"rect_kwargs": {"color": BLUE, "buff": SMALL_BUFF}}
    ...
    def activate_layer(self, layer, activation_vector):
        ...
        if len(av) > n_neurons:
            if self.average_shown_activation_of_large_layer:
                ...
                av = np.append(av[: n_neurons / 2], av[-n_neurons / 2 :])
...
class NetworkScene(Scene):
    CONFIG = {"layer_sizes": [8, 2, 1, 1], "network_mob_config": {}}
...
class ExampleThrees(PiCreatureScene):
    ...
    def get_three_mobs(self):
        ...
        for three_array in three_arrays:
            im_mob = ImageMobject(layer_to_image_array(three_array), height=4)
            pixel_mob = PixelsAsSquares(im_mob)
            three_mob = WrappedImage(pixel_mob, color=LIGHT_GREY, buff=0)
...
class WriteAProgram(Scene):
    def construct(self):
        ...
        self.play(ShowCreation(arrow), LaggedStartMap(FadeIn, choices))
...
class LayOutPlan(TeacherStudentsScene, NetworkScene):
    ...
    def show_words(self):
        ...
        self.play(words[0].shift, MED_LARGE_BUFF * UP, FadeIn(words[1]))
        self.change_student_modes(*["pondering"] * 3, look_at_arg=words)
    ...
    def show_network(self):
        ...
        self.play(
            ...
            self.get_student_changes(*["confused"] * 3, lag_ratio=0),
            self.teacher.change,
            "plain",
            run_time=1,
        )
        self.play(
            ShowCreation(
                network_mob.edge_groups, lag_ratio=0.5, run_time=2, rate_func=linear
            )
        )
    ...
    def show_math(self):
        ...
        equation.set_color_by_tex_to_color_map({"\\textbf{a}": GREEN})
    ...
    def ask_about_layers(self):
        self.student_says(
            "Why the layers?", student_index=2, bubble_kwargs={"direction": LEFT}
        )
        self.wait()
...
class PreviewMNistNetwork(NetworkScene):
    CONFIG = {"n_examples": 15, "network_mob_config": {}}
    ...
    def reset_display(self, answer_rect, image, image_rect):
        ...
        self.play(
            FadeOut(image), FadeOut(image_rect), self.network_mob.deactivate_layers
        )
    ...
    def get_image_to_layer_one_animation(self, image, start_neurons):
        ...
        return Transform(image_mover, start_neurons, remover=True, run_time=1)
...
class AlternateNeuralNetworks(PiCreatureScene):
    def construct(self):
        ...
        self.wait()
        self.play(
            examples[0].shift, MED_LARGE_BUFF * UP, FadeIn(examples[1], lag_ratio=0.5)
        )
        self.wait()
...
class BreakDownName(Scene):
    ...
    def ask_questions(self):
        ...
        name.to_edge(UP)
        q1 = TextMobject("What are \\\\ the ", "neuron", "s?", arg_separator="")
        ...
        self.wait()
        self.play(
            Write(q1, run_time=1), ShowCreation(a1), name[0].set_color, q1.get_color()
        )
        self.play(
...
class IntroduceEachLayer(PreviewMNistNetwork):
    ...
    def show_activation_of_one_neuron(self):
        ...
        def change_activation(num):
            self.play(
                ...
                num,
                ChangingDecimal(
                    example_num, lambda a: example_neuron.get_fill_opacity()
                ),
                UpdateFromFunc(
                    example_num,
                    lambda m: m.set_fill(
    ...
    def transform_into_full_network(self):
        ...
        self.play(
            ReplacementTransform(VGroup(*neurons[:n]), VGroup(*layer.neurons[:n])),
            ReplacementTransform(VGroup(*neurons[n:-n]), layer.dots),
            ReplacementTransform(VGroup(*neurons[-n:]), VGroup(*layer.neurons[-n:])),
        )
        self.play(
        ...
        for edge_group, layer in zip(network_mob.edge_groups, network_mob.layers[1:]):
            self.play(
                LaggedStartMap(FadeIn, layer, run_time=1), ShowCreation(edge_group)
            )
        self.wait()
    ...
    def show_output_layer(self):
        ...
        def change_activation(num):
            self.play(
                ...
                None,
                num,
                ChangingDecimal(activation, lambda a: neuron.get_fill_opacity()),
                UpdateFromFunc(
                    activation,
                    lambda m: m.set_fill(
        ...
        self.wait()
        self.play(MoveToTarget(neuron), MoveToTarget(label))
...
class DiscussChoiceForHiddenLayers(TeacherStudentsScene):
    def construct(self):
        network_mob = MNistNetworkMobject(
            layer_to_layer_buff=2.5, neuron_stroke_color=WHITE
        )
        ...
        self.wait()
        self.student_says("Why 16?", student_index=0, run_time=1)
        self.play(neurons_anim, run_time=3)
        self.play(
            self.teacher.change, "shruggie", RemovePiCreatureBubble(self.students[0])
        )
        self.wait()


class MoreHonestMNistNetworkPreview(IntroduceEachLayer):
    CONFIG = {"network_mob_config": {"edge_propogation_time": 1.5}}
...
class AskAboutPropogationAndTraining(TeacherStudentsScene):
    def construct(self):
        self.student_says(
            "How does one layer \\\\ influence the next?", student_index=0, run_time=1
        )
        self.wait()
        self.student_says("How does \\\\ training work?", student_index=2, run_time=1)
        self.wait(3)


class AskAboutLayers(PreviewMNistNetwork):
    def construct(self):
        self.play(self.network_mob.scale, 0.8, self.network_mob.to_edge, DOWN)
...
class BreakUpMacroPatterns(IntroduceEachLayer):
    ...
    def setup_needed_patterns(self):
        ...
        vects = [
            np.array(Image.open(get_full_raster_image_path("handwritten_" + p)))[
                :, :, 0
            ].flatten()
            / 255.0
            for p in prefixes
    ...
    def show_nine(self):
        ...
        self.wait()
        self.play(Write(equation[3]), right_line[1].restore, FadeIn(right_line[0]))
        self.wait()
    ...
    def show_eight(self):
        ...
        self.play(FadeIn(eight), Write(equation[1]))
        self.play(ReplacementTransform(self.upper_loop.copy(), upper_loop))
        self.wait()
        self.play(FadeIn(lower_loop[1]))
        self.play(Write(equation[3]), lower_loop[1].restore, FadeIn(lower_loop[0]))
        self.wait()
    ...
    def show_second_to_last_layer(self):
        everything = VGroup(
            *it.chain(self.nine_equation, self.eight_equation, self.four_equation)
        )
        patterns = VGroup(
            self.upper_loop,
            self.lower_loop,
        ...
        self.play(
            FadeIn(network_mob, lag_ratio=0.5, run_time=3), MoveToTarget(patterns)
        )
        self.wait(2)
    ...
    def get_equation(self, *mobs):
        equation = VGroup(
            mobs[0],
            TexMobject("=").scale(2),
            *list(it.chain(*[[m, TexMobject("+").scale(2)] for m in mobs[1:-1]]))
            + [mobs[-1]]
        )
...
class GenerallyLoopyPattern(Scene):
    def construct(self):
        image_map = get_organized_images()
        images = list(map(MNistMobject, it.chain(image_map[8], image_map[9])))
...
class SecondLayerIsLittleEdgeLayer(IntroduceEachLayer):
    ...
    def ask_question(self):
        question = TextMobject("Does the network \\\\ actually do this?")
    ...
    def set_neurons_target(self, neurons, layer):
        ...
        Transform(VGroup(*neurons.target[:n]), VGroup(*layer.neurons[:n])).update(1)
        Transform(VGroup(*neurons.target[-n:]), VGroup(*layer.neurons[-n:])).update(1)
        Transform(
    ...
    def separate_parts(self, image_group):
        ...
        self.play(
            MoveToTarget(
                image_group, rate_func=there_and_back, lag_ratio=0.5, run_time=2
            )
        )
...
class ManyTasksBreakDownLikeThis(TeacherStudentsScene):
    def construct(self):
        ...
        syllables = TextMobject("$\\cdot$".join(["re", "cog", "ni", "tion"]))
        word = TextMobject("re", "cognition", arg_separator="")
...
class AskAboutWhatEdgesAreDoing(IntroduceEachLayer):
    ...
    def add_question(self):
        ...
        question = TextMobject("What are these connections actually doing?")
        question.to_edge(UP)
...
class IntroduceWeights(IntroduceEachLayer):
    CONFIG = {"weights_color": GREEN, "negative_weights_color": RED}
    ...
    def show_desired_pixel_region(self):
        ...
        self.play(FadeIn(rect), ShowCreation(pixels, lag_ratio=0.5, run_time=2))
    ...
    def show_weights(self):
        ...
        for p_label in p_labels:
            w_label = TexMobject(p_label.get_tex_string().replace("p", "w"))
    ...
    def show_weighted_sum(self):
        ...
        a_labels = VGroup(*[TexMobject("a_%d" % d) for d in range(1, 5)])

        weighted_sum = VGroup(
            *it.chain(*[[w, a, TexMobject("+")] for w, a in zip(w_labels, a_labels)])
        )
        weighted_sum.add(
            TexMobject("\\cdots"),
            TexMobject("+"),
...
class MotivateSquishing(Scene):
    ...
    def show_real_number_line(self):
        ...
        arrow1, arrow2 = [
            Arrow(weighted_sum.get_bottom(), number_line.number_to_point(n))
            for n in (-3, 3)
        ]
    ...
    def show_interval(self):
        ...
        self.play(GrowFromCenter(brace), GrowFromCenter(interval))
...
class IntroduceSigmoid(GraphScene):
    ...
    def add_title(self):
        ...
        equation = TexMobject("\\sigma(%s) = \\frac{1}{1+e^{-%s}}" % (char, char))
    ...
    def show_part(self, x_min, x_max, color):
        line, graph_part = [
            self.get_graph(func, x_min=x_min, x_max=x_max, color=color).set_stroke(
                width=4
            )
...
class IncludeBias(IntroduceWeights):
    ...
    def add_sigmoid_label(self):
        ...
        self.play(Write(name), ShowCreation(arrow))
        self.sigmoid_name = name
    ...
    def words_on_activation(self):
        ...
        self.play(FadeIn(activation_word), neuron.set_fill, WHITE, 0.8)
        self.wait()
        self.play(GrowArrow(arrow), ReplacementTransform(activation_word, words))
        self.wait(2)
    ...
    def add_bias(self):
        ...
        self.play(
            ReplacementTransform(self.gt_ten.copy(), bias, run_time=2), MoveToTarget(rp)
        )
        self.wait(2)
        self.play(ShowCreation(rect), Write(name))
        self.wait(2)
...
class ShowRemainingNetwork(IntroduceWeights):
    ...
    def show_all_of_second_layer(self):
        ...
        self.play(
            LaggedStartMap(
                ShowCreation, VGroup(*[n.edges_in for n in neurons[7:]]), run_time=3
            ),
            LaggedStartMap(FadeIn, VGroup(*neurons[7:]), run_time=3),
    ...
    def count_in_biases(self):
        ...
        self.play(
            FadeIn(words), LaggedStartMap(GrowArrow, arrows, run_time=3, lag_ratio=0.3)
        )
        self.wait()
    ...
    def show_remaining_layers(self):
        ...
        added_weights = TexMobject("+16\\!\\times\\! 16 + 16 \\!\\times\\! 10")
        ...
        self.play(
            ...
            LaggedStartMap(ShowCreation, edges, run_time=4, lag_ratio=0.3),
            LaggedStartMap(FadeIn, neurons, run_time=4, lag_ratio=0.3),
        )
        self.wait(2)
    ...
    def show_final_number(self):
        group = VGroup(self.weights_count, self.bias_count)
        ...
        self.play(ShowCreation(rect), MoveToTarget(group))
    ...
    def tweak_weights(self):
        ...
        finding_words = TextMobject("Finding the right \\\\ weights and biases")
    ...
    def get_edge_weight_wandering_anim(self, edges):
        ...
        self.play(
            LaggedStartMap(MoveToTarget, edges, lag_ratio=0.6, run_time=2), *added_anims
        )
...
class WhenTheNetworkFails(MoreHonestMNistNetworkPreview):
    ...
    def black_box(self):
        ...
        box = SurroundingRectangle(
            layers, stroke_color=WHITE, fill_color=BLACK, fill_opacity=0.8
        )
    ...
    def ask_about_weights(self):
        question = TextMobject(
            "What weights are used here?\\\\", "What are they doing?"
        )
...
class IntroduceWeightMatrix(NetworkScene):
    ...
    def fade_many_neurons(self):
        ...
        anims += [Animation(neurons[0]), Animation(neurons[0].edges_in)]
        self.play(*anims)
    ...
    def activate_first_layer(self):
        ...
        a_labels = VGroup(
            *[TexMobject("a^{(0)}_%d" % d) for d in range(len(layer.neurons))]
        )
    ...
    def show_first_neuron_weighted_sum(self):
        ...
        w_labels = VGroup(*[TexMobject("w_{0, %d}" % d) for d in range(len(a_labels))])
        weighted_sum = VGroup()
        symbols = VGroup()
        ...
        self.play(Write(w_labels), Write(symbols))
    ...
    def add_bias(self):
        ...
        self.play(FadeIn(name), FadeIn(bias), GrowArrow(arrow))
    ...
    def organize_activations_into_column(self):
        ...
        last_a = column[-1]
        new_last_a = TexMobject(last_a.get_tex_string().replace("7", "n"))
    ...
    def organize_weights_as_matrix(self):
        ...
        row_1, row_k = [
            VGroup(
                *list(
                    map(
                        TexMobject,
                        ["w_{%s, 0}" % i, "w_{%s, 1}" % i, "\\cdots", "w_{%s, n}" % i],
                    )
                )
            )
            for i in ("1", "k")
        ...
        dots_row = VGroup(
            *list(map(TexMobject, ["\\vdots", "\\vdots", "\\ddots", "\\vdots"]))
        )
        ...
        self.play(FadeIn(lower_rows, run_time=3, lag_ratio=0.5))
        self.wait()
    ...
    def connect_weighted_sum_to_matrix_multiplication(self):
        ...
        self.play(GrowArrow(arrow), GrowFromCenter(brace))
        self.play(Write(equals), FadeIn(result_brackets))
    ...
    def add_bias_vector(self):
        ...
        b_column = VGroup(*list(map(TexMobject, ["b_0", "b_1", "\\vdots", "b_n"])))
        b_column.scale(0.85)
    ...
    def write_clean_final_expression(self):
        ...
        self.play(
            ReplacementTransform(
                VGroup(self.a_column, self.a_column_brackets).copy(), VGroup(VGroup(a0))
            )
        )
        self.play(
            ReplacementTransform(
...
class HorrifiedMorty(Scene):
    def construct(self):
        ...
        for mode in "horrified", "hesitant":
            self.play(morty.change, mode, morty.look, UP)
...
class NeuronIsFunction(MoreHonestMNistNetworkPreview):
    ...
    def neuron_is_function(self):
        ...
        self.play(FadeIn(function_word), VGroup(thing_words, cross).to_edge, DOWN)
        self.wait()
    ...
    def show_neuron_as_function(self):
        ...
        self.play(
            LaggedStartMap(
                ShowCreationThenDestruction, edges.copy().set_stroke(YELLOW, 4)
            )
        )
    ...
    def fade_network_back_in(self):
        ...
        anims += [FadeOut(self.neuron_arrow), FadeOut(self.decimal)]
...
class NextVideo(MoreHonestMNistNetworkPreview, PiCreatureScene):
    ...
    def talk_about_subscription(self):
        ...
        subscribe_word = TextMobject("Subscribe", "!", arg_separator="")
        ...
        self.play(
            Write(subscribe_word), self.video.shift, 3 * LEFT, MoveToTarget(morty)
        )
...
class IntroduceReLU(IntroduceSigmoid):
    ...
    def show_ReLU(self):
        graph = VGroup(
            Line(self.coords_to_point(-7, 0), self.coords_to_point(0, 0)),
            Line(self.coords_to_point(0, 0), self.coords_to_point(4, 4)),
        )
...
class CompareSigmoidReLUOnDeepNetworks(PiCreatureScene):
    def construct(self):
        ...
        sigmoid_graph = FunctionGraph(sigmoid, x_min=-5, x_max=5)
        ...
        slow_arrow = Arrow(slow_learner.get_bottom(), sigmoid_graph.get_top())

        relu_graph = VGroup(
            Line(2 * LEFT, ORIGIN), Line(ORIGIN, np.sqrt(2) * (RIGHT + UP))
        )
        ...
        network_mob.shift(RIGHT)
        edge_update = ContinualEdgeUpdate(network_mob, stroke_width_exp=1)
...
class Thumbnail(NetworkScene):
    ...
    def construct(self):
        ...
        subtitle = TextMobject("From the\\\\", "ground up\\\\")
        # subtitle.arrange(
        #     DOWN,
        #     buff=0.25,
        #     aligned_edge=LEFT,
        ...
        edge_update = ContinualEdgeUpdate(
            network_mob, max_stroke_width=10, stroke_width_exp=4
        )
